...
branin.runshell.vec <- function(X,...) {
    unlist(Funz_Run(model=CODE,input.files=SRC,input.variables = list(x1=X[,1],x2=X[,2]),verbosity=VERBOSITY,archive.dir="/tmp",...)$cat)
}
...
testMatchRef <- function() {
    ref = Funz_Design(design = DESIGN, options = DESIGN_OPT,input.variables = list(x1="[0,1]",x2="[0,1]"), fun = branin.ref.vec,verbosity=VERBOSITY,archive.dir="/tmp")
    runshell = Funz_Design(design = DESIGN, options = DESIGN_OPT,input.variables = list(x1="[0,1]",x2="[0,1]"), fun = branin.runshell.vec,verbosity=VERBOSITY,archive.dir="/tmp")
...
testVectorizeFun <- function(f = branin.ref.vec) {
    ref = Funz_Design(design = DESIGN, options = DESIGN_OPT,input.variables = list(x1="[0,1]",x2="[0,1]"), fun = f,verbosity=VERBOSITY,fun.control=list(vectorize="fun"),archive.dir="/tmp")
...
testNoVectorize <- function(f = branin.ref.novec) {
    ref = Funz_Design(design = DESIGN, options = DESIGN_OPT,input.variables = list(x1="[0,1]",x2="[0,1]"), fun = f,verbosity=VERBOSITY,fun.control=list(vectorize=NULL),archive.dir="/tmp")
...
testVectorizeForeach <- function(f = branin.ref.novec) {
    require(doMC)
    registerDoMC()
    ref = Funz_Design(design = DESIGN, options = DESIGN_OPT,input.variables = list(x1="[0,1]",x2="[0,1]"), fun = f,verbosity=VERBOSITY,fun.control=list(vectorize="foreach"),archive.dir="/tmp")
...
testVectorizeParallel <- function(f = branin.ref.novec) {
    ref <<- Funz_Design(design = DESIGN, options = DESIGN_OPT,input.variables = list(x1="[0,1]",x2="[0,1]"), fun = f,verbosity=VERBOSITY,fun.control=list(vectorize="parallel"),archive.dir="/tmp")
