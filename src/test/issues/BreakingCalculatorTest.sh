...
ok1=`ps aux | grep $PID_RUN | grep Funz.sh | wc -l`
...
ok4=`ps aux | grep $PID_RUN | grep Funz.sh | wc -l`
...
ok5=`ps aux | grep $PID_RUN | grep Funz.sh | wc -l`
...
i=0
while [ $i -le 10 ]; do
  ok3=`ps aux | grep $PID_RUN | grep Funz.sh | wc -l`
  if [ $ok3 = "0" ]; then
      break
  fi
  i=$(( $i + 1 ))
...
ok6=`ps aux | grep $PID_RUN | grep Funz.sh | wc -l`
