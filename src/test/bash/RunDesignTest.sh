...
function testRunDesignParseError {
    ...
    if (($ok > 0)); then rm testRunDesignParseError.out; echo "OK";return 0; else echo "FAILED:"; cat testRunDesignParseError.out;return 1; fi
}

function testRun1Design {
    $FUNZ_HOME/Funz.sh RunDesign -m $CODE -d $DESIGN -do nmax=$NMAX delta=$DELTA -if $TMP_IN -iv x1=.3 x2=[0.3,.4] -v $VERBOSITY -ad tmp/testRun1Design.sh > testRun1Design.out 2>&1
    ok=`grep "minimum is " testRun1Design.out | wc -l`
    if (($ok > 0)); then rm testRun1Design.out; echo "OK";return 0; else echo "FAILED:"; cat testRun1Design.out;return 1; fi
}

function testRun2Design {
    $FUNZ_HOME/Funz.sh RunDesign -m $CODE -d $DESIGN -do nmax=$NMAX delta=$DELTA -if $TMP_IN -iv x1=.3,.5 x2=[0.3,.4] -v $VERBOSITY -ad tmp/testRun2Design.sh > testRun2Design.out 2>&1
    ok=`grep "minimum is " testRun2Design.out | wc -l`
    if (($ok > 0)); then rm testRun2Design.out; echo "OK";return 0; else echo "FAILED:"; cat testRun2Design.out;return 1; fi
}
...
function testBraninGradientDescent {
    $FUNZ_HOME/Funz.sh RunDesign -m $CODE -d $DESIGN -do nmax=$NMAX delta=$DELTA -if $TMP_IN -iv x1=[0,1] x2=[0,1] -v $VERBOSITY -ad tmp/testBraninGradientDescent.sh > testBraninGradientDescent.out 2>&1
    ok=`grep "$BraninGradientDescent_MIN" testBraninGradientDescent.out | wc -l`
    if (($ok > 0)); then rm testBraninGradientDescent.out; echo "OK";return 0; else echo "FAILED:"; cat testBraninGradientDescent.out;return 1; fi
}
...
function testBraninGradientDescentx2 {
    ...
    if (($ok1 > 0)) && (($ok2 > 0)); then rm testBraninGradientDescentx2.out; echo "OK";return 0; else echo "FAILED:"; cat testBraninGradientDescentx2.out;return 1; fi
}
