...
function testRun1Design {
    $FUNZ_HOME/Funz.sh RunDesign -m $CODE -d $DESIGN -do nmax=$NMAX delta=$DELTA -if $TMP_IN -iv x1=.3 x2=[0.3,.4] -v $VERBOSITY -ad tmp > testRun1Design.out 2>&1
...
function testRun2Design {
    $FUNZ_HOME/Funz.sh RunDesign -m $CODE -d $DESIGN -do nmax=$NMAX delta=$DELTA -if $TMP_IN -iv x1=.3,.5 x2=[0.3,.4] -v $VERBOSITY -ad tmp > testRun2Design.out 2>&1
...
function testRunDesignFailed {
    $FUNZ_HOME/Funz.sh RunDesign -m $CODE -d $DESIGN -do nmax=$NMAX delta=$DELTA -if $TMP_IN -iv x1=[-1.1,1] x2=[0.3,.4] -v $VERBOSITY -ad tmp -oe x1+min\(cat,10,na.rm=F\) > testRunDesignFailed.out 2>&1
...
function testBraninGradientDescent {
    $FUNZ_HOME/Funz.sh RunDesign -m $CODE -d $DESIGN -do nmax=$NMAX delta=$DELTA -if $TMP_IN -iv x1=[0,1] x2=[0,1] -v $VERBOSITY -ad tmp > testBraninGradientDescent.out 2>&1
...
function testBraninGradientDescentx2 {
    $FUNZ_HOME/Funz.sh RunDesign -m $CODE -d $DESIGN -do nmax=$NMAX delta=$DELTA -if $TMP_IN -iv x1=0,1 x2=[0,1] -v $VERBOSITY -ad tmp > testBraninGradientDescentx2.out 2>&1
