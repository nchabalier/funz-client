...
function testRunDesignParseError {
    ...
    if [ $ok = "1" ]; then rm testRunDesignParseError.out; echo "OK"; else echo "FAILED:"; cat testRunDesignParseError.out; fi
}
...
function testRun1Design {
    ...
    if [ $ok = "1" ]; then rm testRun1Design.out; echo "OK"; else echo "FAILED:"; cat testRun1Design.out; fi
}
...
function testRun2Design {
    ...
    if [ $ok = "2" ]; then rm testRun2Design.out; echo "OK"; else echo "FAILED:"; cat testRun2Design.out; fi
}
...
function testRunDesignFailed {
    ...
    if [ $ok = "1" ]; then rm testRunDesignFailed.out; echo "OK"; else echo "FAILED:"; cat testRunDesignFailed.out; fi
}

function testBraninGradientDescent {
    $FUNZ_HOME/Funz.sh RunDesign -m $CODE -d $DESIGN -do nmax=$NMAX -if $TMP_IN -iv x1=[0,1] x2=[0,1] -v $VERBOSITY -ad tmp > testBraninGradientDescent.out 2>&1
    ok=`grep "$BraninGradientDescent_MIN" testBraninGradientDescent.out | wc -l`
    if [ $ok = "1" ]; then rm testBraninGradientDescent.out; echo "OK"; else echo "FAILED:"; cat testBraninGradientDescent.out; fi
}

function testBraninGradientDescentx2 {
    $FUNZ_HOME/Funz.sh RunDesign -m $CODE -d $DESIGN -do nmax=$NMAX -if $TMP_IN -iv x1=0,1 x2=[0,1] -v $VERBOSITY -ad tmp > testBraninGradientDescentx2.out 2>&1
    ...
    if [ $ok1 = "1" ] && [ $ok2 = "1" ]; then rm testBraninGradientDescentx2.out; echo "OK"; else echo "FAILED:"; cat testBraninGradientDescentx2.out; fi
}
