...
def testBraninGradientDescent() :
    ...
    if not math.fabs(float(rundesign['min'][0])-BraninGradientDescent_MIN)<1e-1 :
        global failed
        failed=1
...
def testBraninGradientDescentx2() :
    ...
    if not (numpy.fabs(numpy.array([ float(m) for m in rundesign['min'] ])-numpy.array([BraninGradientDescentx2_x1_0_MIN,BraninGradientDescentx2_x1_1_MIN]))<1e-1).all():
        global failed
        failed=1
        return("FAILED to match reference and Funz evaluation")
    else : 
