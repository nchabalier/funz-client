...
def testMatchRef() :
    ...
    if not (numpy.fabs(numpy.array(float(runshell['min'])).transpose()-numpy.array(float(ref['min']))) < 1e-1).all() :
        global failed
        failed=1
...
def testVectorizeFun() :
    ...
    if not math.fabs(float(ref['min'])-BraninGradientDescent_MIN)<1e-1 :
        global failed
        failed=1
