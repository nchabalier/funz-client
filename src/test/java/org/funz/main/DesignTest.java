...
public class DesignTest extends org.funz.api.TestUtils {
    ...
    public void testDesignFailedResult() throws Exception {
        ...
        try {
            Design.main("Design -d GradientDescent -do nmax=3 epsilon=0.000000001 delta=0.01 target=-10 -f ./multtttttttt.sh -fa x1 x2 -iv x1=[-0.5,.0001] x2=[-0.3,.8] -v 10".split(" "));
        } catch (ExitCatcher e) {
    ...
    public void testDesign() throws Exception {
        ...
        try {
            Design.main("Design -d GradientDescent -do nmax=3 epsilon=0.000000001 delta=0.01 target=-10 -f ./mult.sh -fa x1 x2 -iv x1=[-0.5,.0001] x2=[-0.3,.8] -v 10 -ad tmp".split(" "));
        } catch (ExitCatcher e) {
    ...
    public void testDesignPar() throws Exception {
        ...
        try {
            Design.main("Design -d GradientDescent -do nmax=3 epsilon=0.000000001 delta=0.01 target=-10 -f ./mult.sh -fa x1 x2 -fp 3 -iv x1=[-0.5,.0001] x2=[-0.3,.8] -v 10".split(" "));
        } catch (ExitCatcher e) {
            assert e.status == 0 : "Bad exit status :" + e.status;
        }
        assert new File("Design.csv").exists() : "No output file Design.csv created";
    ...
    public void testDesignTooMuchPar() throws Exception {
        ...
        try {
            Design.main("Design -d GradientDescent -do nmax=3 epsilon=0.000000001 delta=0.01 target=-10 -f ./mult.sh -fa x1 x2 -fp 5 -iv x1=[-0.5,.0001] x2=[-0.3,.8] -v 10".split(" "));
        } catch (ExitCatcher e) {
            assert e.status == 0 : "Bad exit status :" + e.status;
        }
        assert new File("Design.csv").exists() : "No output file Design.csv created";
