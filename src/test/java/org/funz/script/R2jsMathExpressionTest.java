 ...
 * @author richet
 */
public class R2jsMathExpressionTest {

    RMathExpression engine;

    public static void main(String args[]) {
        new Funz(); // needed to initalize locale/decimal separator
        org.junit.runner.JUnitCore.main(R2jsMathExpressionTest.class.getName());
    }
    ...
    public void setUp() {
        ...
        Configuration.setProperty("R.server", "R2js");
...
//                + "}");
        ...
        engine.R.voidEval("options = list(nmax = 10, epsilon = 0.01, target=0)");
    ...
    public void testSplitEval() throws Exception {
        ...
        engine.R.voidEval("f = function(x){b <- 1+pi \n d <- 1 \n return(d)}");
        assert Arrays.asList(engine.R.ls(true)).contains("f") : "f not found in "+Arrays.asList(engine.R.ls());
        ...
        engine.R.voidEval("ff <- function(){b=1+pi ; 1}");
