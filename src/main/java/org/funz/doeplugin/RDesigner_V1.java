    ...
    public class RDesign extends Design {
            ...
            try {
                ...
                if (R.asLogical(R.eval("exists('displayResultsTmp')"))) {
                    ...
                    try {
                        ...
                        try {
                            if (R.asLogical(R.eval("'files' %in% names(DesignEnv)"))) {
        ...
        public synchronized String displayResults(List<Experiment> experiments) {
            ...
            try {
                ...
                R.savels(new File(_repository.getAbsolutePath(), getName().replace(' ', '_') + (currentiteration) + ".res.Rdata"), "");//(currentiteration) );
